...
import { GridProvider as SquareGridProvider } from './hexagon/context/GridContext';
import { Link, useNavigate } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";

// Mocks for missing dependencies
type GridTemplate = 'square' | 'hexagonal';
...
const useCollage = () => ({
  createGroup: async (data: any) => {
    ...
    console.log("Mock createGroup:", data);
...
const useAuth = () => {
  const [user, setUser] = useState<{ id: string; guidesSeen: Record<string, boolean> } | null>({
    id: "mock-user-123",
    guidesSeen: {}
  });
...
const userApi = {
  getUserGroups: async (id: string) => ({ items: [] })
};

const UserWalkthrough = ({ run, onFinish }: any) => {
  return null;
};
