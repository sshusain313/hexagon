...
const GridBoard: React.FC = () => {
    ...
    const gridConfig = [
        { cells: 3, offset: false },
        { cells: 4, offset: false }, // same width → same offset
        { cells: 5, offset: true },
        { cells: 4, offset: false },
        { cells: 5, offset: true },
        { cells: 4, offset: false },
        { cells: 3, offset: true },
    ...
    const isCellVisible = (rowIndex: number, colIndex: number): boolean => {
        // Remove center cell (index 1) from top row (index 0)
        if (rowIndex === 0 && colIndex === 1) return false;
        if (rowIndex === 6 && colIndex === 1) return false;
        return true;
    };
    ...
    const mergePlan: Record<number, { startIdx: number; count: number }> = useMemo(() => ({
        ...
        1: { startIdx: 1, count: 2 },  // row 6: 3-6
        2: { startIdx: 1, count: 3 },  // row 7: 3-7
        3: { startIdx: 0, count: 4 },  // row 8: 2-7
        4: { startIdx: 1, count: 3 },  // row 9: 3-7
        5: { startIdx: 1, count: 2 },  // row 10: 3-6
    ...
    const handleCenterClick = () => {
        console.log('[GridBoard] Center/outer clicked -> opening file picker');
    ...
    const handleFileChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
        ...
        if (!file) return;
        console.log('[GridBoard] File selected:', file.name, file.type, file.size);
                        ...
                        <g style={{
                            ...
                            <defs>
                                ...
                                <clipPath id="clip-center-bg" clipPathUnits="userSpaceOnUse">
                                    ...
                                    {/* FILLER FOR TOP GAP: Identify top row and fill gaps between adjacent hexes */}
                                    {(() => {
                                        if (hiddenCellPositions.length < 2) return null;

                                        // 1. Group by row (using fuzzy cy)
                                        const rows: Record<string, typeof hiddenCellPositions> = {};
                                        hiddenCellPositions.forEach(pos => {
                                            const rowKey = Object.keys(rows).find(k => Math.abs(parseFloat(k) - pos.cy) < 5);
                                            if (rowKey) {
                                                rows[rowKey].push(pos);
                                            } else {
                                                rows[pos.cy.toString()] = [pos];
                                            }
                                        });

                                        const sortedRowKeys = Object.keys(rows).map(parseFloat).sort((a, b) => a - b);
                                        if (sortedRowKeys.length === 0) return null;

                                        // TOP ROW FILLER
                                        const topRowY = sortedRowKeys[0];
                                        const topRowCells = rows[topRowY.toString()].sort((a, b) => a.cx - b.cx);
                                        const topFillers = topRowCells.map((cell, i) => {
                                            if (i === topRowCells.length - 1) return null;
                                            const nextCell = topRowCells[i + 1];
                                            const OVERLAP_FACTOR = 1.30;
                                            const r1 = (cell.width / 2) * OVERLAP_FACTOR;
                                            const r2 = (nextCell.width / 2) * OVERLAP_FACTOR;

                                            // Top Filler Points
                                            const x1 = cell.cx;
                                            const y1 = cell.cy - r1;
                                            const x2 = nextCell.cx;
                                            const y2 = nextCell.cy - r2;
                                            const x3 = nextCell.cx;
                                            const y3 = nextCell.cy;
                                            const x4 = cell.cx;
                                            const y4 = cell.cy;

                                            const points = `${x1},${y1} ${x2},${y2} ${x3},${y3} ${x4},${y4}`;
                                            return <polygon key={`gap-fill-bg-top-${i}`} points={points} />;
                                        });

                                        // BOTTOM ROW FILLER
                                        const bottomRowY = sortedRowKeys[sortedRowKeys.length - 1];
                                        const bottomRowCells = rows[bottomRowY.toString()].sort((a, b) => a.cx - b.cx);
                                        const bottomFillers = bottomRowCells.map((cell, i) => {
                                            if (i === bottomRowCells.length - 1) return null;
                                            const nextCell = bottomRowCells[i + 1];
                                            const OVERLAP_FACTOR = 1.30;
                                            const r1 = (cell.width / 2) * OVERLAP_FACTOR;
                                            const r2 = (nextCell.width / 2) * OVERLAP_FACTOR;

                                            // Bottom Filler Points
                                            // Bottom Tip 1
                                            const x1 = cell.cx;
                                            const y1 = cell.cy + r1;
                                            // Bottom Tip 2
                                            ...
                                            const points = `${x1},${y1} ${x2},${y2} ${x3},${y3} ${x4},${y4}`;
                                            return <polygon key={`gap-fill-bg-bottom-${i}`} points={points} />;
                                        });

                                        return [...topFillers, ...bottomFillers];
                                    })()}
                                </clipPath>
                            </defs>
                    ...
                    >
                        <defs>
                            ...
                            <clipPath id="clip-center-click" clipPathUnits="userSpaceOnUse">
                                ...
                                {/* FILLER FOR TOP AND BOTTOM GAP (Click Layer) */}
                                {(() => {
                                    if (hiddenCellPositions.length < 2) return null;
                                    ...
                                    hiddenCellPositions.forEach(pos => {
                                        const rowKey = Object.keys(rows).find(k => Math.abs(parseFloat(k) - pos.cy) < 5);
                                        if (rowKey) rows[rowKey].push(pos);
                                        else rows[pos.cy.toString()] = [pos];
                                    });
                                    ...
                                    const topFillers = topRowCells.map((cell, i) => {
                                        ...
                                        // Standard radius for click layer
                                        const r1 = cell.width / 2;
                                        const r2 = nextCell.width / 2;
                                        ...
                                        return <polygon key={`gap-fill-click-top-${i}`} points={`${x1},${y1} ${x2},${y2} ${x3},${y3} ${x4},${y4}`} />;
                                    });
                                    ...
                                    const bottomFillers = bottomRowCells.map((cell, i) => {
                                        ...
                                        return <polygon key={`gap-fill-click-bottom-${i}`} points={`${x1},${y1} ${x2},${y2} ${x3},${y3} ${x4},${y4}`} />;
                                    });
